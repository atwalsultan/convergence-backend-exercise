...
export async function updateTaskHandler(req: Request, res: Response) {
  ...
  if (String(task.user) !== userId) return res.sendStatus(401);
...
export async function getFilteredTasksHandler(req: Request, res: Response) {
  ...
  if (!filteredTasks) return res.status(400).send("Please select a valid filter (done or pending)");
...
export async function deleteTaskHandler(req: Request, res: Response) {
  ...
  if (String(task.user) !== String(userId)) return res.sendStatus(401);
